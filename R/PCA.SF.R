####PCA.SF####
PD.PCAplot<-function(xdata, ydata, ColObj, title, pointsize=1.5, level=0.95, legendtitle="group", labels=NULL){
#library(ggplot2)
if(!is.null(labels)){if(length(labels)!=length(xdata)){stop("problem with labels")}}
...
if(is.null(labels)){
  ...
  g <- g + geom_text_repel(label=labels)
