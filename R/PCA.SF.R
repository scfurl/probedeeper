...
g<-ggplot2::ggplot(data=dfr, aes(x=x, y=y)) + ggplot2::geom_point(colour=patientcolors, size=1.5, alpha=1)+
  ggplot2::geom_point(data=centroids, aes(x=x, y=y), colour=colors, size=3, shape=43) +
  ggplot2::geom_segment(aes(x=x.centroid, y=y.centroid, xend=x, yend=y), colour=patientcolors, size=0.2)+
...
for(i in 1:length(levels(as.factor(dfr_ell$group)))){
  g<-g+ggplot2::geom_path(data=dfr_ell[dfr_ell$group==levels(as.factor(dfr_ell$group))[i],], aes(x=x, y=y), size=0.2, colour=levels(factor(patientcolors, levels = unique(patientcolors)))[i], linetype=1)
  g<-g+ggplot2::geom_polygon(data=dfr_ell[dfr_ell$group==levels(as.factor(dfr_ell$group))[i],], aes(x=x, y=y), fill=levels(factor(patientcolors, levels = unique(patientcolors)))[i], alpha=0.1, linetype=0) }
